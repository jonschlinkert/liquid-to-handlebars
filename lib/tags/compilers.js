    ...
    compiler
      ...
      .set('expression', function(node) {
        ...
        if (!skip && /[\|\[\]]/.test(node.args)) {
          var value = node.args;
          var segs = split(value, {sep: ' ', keepQuotes: true, brackets: true});
          var rest = '';
          ...
          var arr = split(val, {sep: ' ', keepQuotes: true, brackets: true});
          tight = (!node.hasClose && arr.length > 1) || utils.isKeyword(val);
        } else {
