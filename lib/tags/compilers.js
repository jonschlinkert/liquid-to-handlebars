  ...
  return function(compiler) {
    ...
    compiler
      ...
      .set('tag', function(node) {
        if (node.name.slice(0, 3) === 'end') {
          nameStack.pop();
        } else if (node.hasClose) {
      ...
      .set('expression', function(node) {
        ...
        if (nameStack.length && options.addScopes === true) {
          for (let i = 1; i < nameStack.length; i++) {
            val = '../' + val;
          }
        }

        // maybe prefix "item" in {% for item in array %} with "@"
        if (options.prefix && last && last.tok && _val === last.tok.key) {
          if (val[0] !== options.prefix) {
