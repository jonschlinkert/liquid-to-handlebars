...
utils.hasEndTag = function(name, str) {
  ...
  if (/^(assign|cycle|else|elsif|include)$/.test(name)) {
    return false;
  }

  if (/^(capture|case|comment|for|if|paginate|raw|tablerow|unless)$/.test(name)) {
    return true;
  }
...
utils.namespaceArgs = function(args, names, options) {
  ...
  if (isString) {
    args = split(args, {sep: ' ', keepQuotes: true, brackets: true});
  }
